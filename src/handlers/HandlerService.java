...
public class HandlerService {
 ...
 //       LDRAuthorizerView.globalSessionManager.getSessionIdManager().
    ...
    //   HttpSession xexion = baseRequest.getSession();
        ...
        //****************** RESET PORTFOLIO
        if (string.startsWith("/service/resetPortfolio")) {
            LDRServices.resetPortfolio(baseRequest, request, response);
        }
        
        //****************** FAKE PAYMENT
        if (string.startsWith("/service/fakePayment")) {
            LDRServices.fakePayment(baseRequest, request, response);
        }

        //****************** SHOW PAYMENT
        if (string.startsWith("/service/showPayment")) {
        ...
        //****************** GET DATASET
        if (string.startsWith("/service/getDataset")) {
            LDRServices.getDataset(baseRequest, request, response);
        }

        //****************** EXPORT PORTFOLIO
        if (string.startsWith("/service/exportPortfolio")) {
