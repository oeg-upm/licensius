...
public class CKANExplorer {
    ...
    public static void othermain(String[] args) {
        ...
        getMiniJSONRapido("http://datahub.io/api/3/search/dataset?all_fields=1&rows=1000&start=0&res_format=example%2Frdf%2Bxml", "d1.json");
...
//            String sjson=(String)obj;
    ...
    public static List<String> getDatasetsFromAGroup(String grupo) {
        ...
        try {
            String uri = "http://datahub.io/api/3/action/member_list?id=" + grupo;
            URL url = new URL(uri);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("GET");
            conn.setRequestProperty("Accept", "application/json");
